...
void handleRoot() {
	String html = "<!DOCTYPE html><html><head><title>Data Input</title>";
	html += "<style>";
	html += "body { font-family: Arial, sans-serif; margin: 50px; }";
	html += "h2 { color: #333; }";
	html += "label { display: block; margin-top: 10px; }";
	html += "input[type='text'] { width: 200px; padding: 5px; margin-top: 5px; }";
	html += "input[type='submit'] { width: 100px; padding: 8px; margin-top: 20px; background-color: #4CAF50; color: white; border: none; cursor: pointer; }";
	html += "</style>";
	html += "</head><body>";
	html += "<h2>Enter Bus Number, Bus Stop, Korean Stock, US Stock, WiFi Name, and WiFi Password</h2>";
	html += "<form action='/submitData' method='post' id='dataForm'>";
	html += "<label for='busNumber'>Bus Number:</label>";
	html += "<input type='text' id='busNumber' name='busNumber'><br>";
	html += "<label for='busStop'>Bus Stop:</label>";
	html += "<input type='text' id='busStop' name='busStop'><br>";
	html += "<label for='koreanStock1'>Korean Stock 1:</label>";
	html += "<input type='text' id='koreanStock1' name='koreanStock1'><br>";
	html += "<label for='koreanStock2'>Korean Stock 2:</label>";
	html += "<input type='text' id='koreanStock2' name='koreanStock2'><br>";
	html += "<label for='koreanStock3'>Korean Stock 3:</label>";
	html += "<input type='text' id='koreanStock3' name='koreanStock3'><br>";
	html += "<label for='usStock1'>US Stock 1:</label>";
	html += "<input type='text' id='usStock1' name='usStock1'><br>";
	html += "<label for='usStock2'>US Stock 2:</label>";
	html += "<input type='text' id='usStock2' name='usStock2'><br>";
	html += "<label for='usStock3'>US Stock 3:</label>";
	html += "<input type='text' id='usStock3' name='usStock3'><br>";
	html += "<label for='wifiName'>WiFi Name:</label>";
	html += "<input type='text' id='wifiName' name='wifiName'><br>";
	html += "<label for='wifiPassword'>WiFi Password:</label>";
	html += "<input type='text' id='wifiPassword' name='wifiPassword'><br>";
...
void handleData() {
	if (server.hasArg("busNumber") && server.hasArg("busStop") && server.hasArg("koreanStock1") && server.hasArg("koreanStock2") && server.hasArg("koreanStock3") && server.hasArg("usStock1") && server.hasArg("usStock2") && server.hasArg("usStock3") && server.hasArg("wifiName") && server.hasArg("wifiPassword")) {
		[
		String busNumber = server.arg("busNumber");
		String busStop = server.arg("busStop");
		String koreanStock1 = server.arg("koreanStock1");
		String koreanStock2 = server.arg("koreanStock2");
		String koreanStock3 = server.arg("koreanStock3");
		String usStock1 = server.arg("usStock1");
		String usStock2 = server.arg("usStock2");
		String usStock3 = server.arg("usStock3");
		...
		myStockUS.name[2][sizeof(myStockUS.name[2]) - 1] = '\0'; // null-terminate
